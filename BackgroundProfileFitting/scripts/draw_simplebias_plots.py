    ...
    for truth in truth_mods:
      for test in test_mods:
        for plot in plot_types:
          graphs[(cat,truth,test,plot)]=drawGraph(cat,truth,test,plot,masses)
    ...
    for truth in truth_mods:
      for plt in plot_types:
        ...
        for format in [".png",".pdf"]:
          c.SaveAs("%s/cat%d_truth_%s_muInj_0.0_%s_median%s"%(options.outdir,int(cat.lstrip('cat')),truth,plt,format))
          

def drawGraph(cat=0,truth="exp1",test="exp1",plot="pull",masses=[110,120,130,140,150]):
  file = r.TFile.Open(options.infile)
  histos={}
  plotType=plot
  if (plot=="pull"):
  ...
  graphDict['median'].SetName("cat%d_truth_%s_test_%s_muInj_0.0_median"%(int(cat.lstrip('cat')),truth,test))
  graphDict['mean'].SetName("cat%d_truth_%s_test_%s_muInj_0.0_mean"%(int(cat.lstrip('cat')),truth,test))
  graphDict['rmsband'].SetName("cat%d_truth_%s_test_%s_muInj_0.0_rmsband"%(int(cat.lstrip('cat')),truth,test))
  graphDict['band68'].SetName("cat%d_truth_%s_test_%s_muInj_0.0_band68"%(int(cat.lstrip('cat')),truth,test))
  graphDict['band95'].SetName("cat%d_truth_%s_test_%s_muInj_0.0_band95"%(int(cat.lstrip('cat')),truth,test))

  i=0
  print "+++++++ PLOT_TYPE "+str(plot)
  for mass in masses:
    h=file.Get("cat%d_truth_%s_test_%s_muInj_0.0_mass_%d_%s"%(int(cat.lstrip('cat')),truth,test,int(mass),plotType))
    print h.GetName()
